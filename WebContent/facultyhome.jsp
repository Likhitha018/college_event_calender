...
<title>St.Mary's Engineering College Annual Calendar</title>
...
var noMessage =  '<h2>Important Notifications</h2>';
...
while(rs3.next())
{
    ...
    String[] s=rs3.getString(3).split("-");
...
for(dex=0; dex < 7; dex++) 
{
	...
	} cal += '<\/tr><tr>'; var day2 = today; for (dex = today; dex > 6; dex -=7) day2 = dex; weekday -= day2 - 1; 
	...
	for(dex=weekday; dex < 7; dex++) cal += td_start + ' <\/td>'; cal += '<\/tr><\/table><\/div>';if (document.getElementById) {var mmb = month;  mm = month + 1; var yya = yyb = yy; if (mmb <1) {mmb += 12; yyb--;} var mma = month + 2; if (mma > 12) {mma -= 12; yya++;} var yb = yy -1; var ya = yy +1; cal += '<table border="0" cellspacing="0" cellpadding="2" width="210"><tr><td><a href="#" onclick="if (cala = dispCal('+yb+','+mm+')) {document.getElementById(\'cal\').innerHTML = cala; return false;}"><<</a></td><td><a href="#" onclick="if (cala = dispCal('+yyb+','+mmb+')) {document.getElementById(\'cal\').innerHTML = cala; return false;}"><</a></td><td align="right"><a href="#" onclick="if (cala = dispCal('+yya+','+mma+')) {document.getElementById(\'cal\').innerHTML = cala; return false;}">></a></td><td align="right"><a href="#" onclick="if (cala = dispCal('+ya+','+mm+')) {document.getElementById(\'cal\').innerHTML = cala; return false;}">>></a></td></tr></table>';} else {cal += '<div> </div>';} cal += '</div>'; return cal;}
function start() {var x = '<div id="calDate" style="border:1pt ;width:238px"><\/div>'; var y = ''; if (tb == 't') y = x + dispCal(0,0); else y = dispCal(0,0) + x; document.getElementById('calendar').innerHTML = y; ev();}
loaded('calendar',start);
function clickDate(day, month, year) {var ct = nextDate(year + getmmdd(month+1,day));if (ct == '') ct = nextDate('0000' + getmmdd(month+1,day));if (ct == '') return 'style=""'; else return 'style="cursor:pointer;color:'+linkclr+'" onclick="selDate = '+year+ getmmdd(month+1,day)+'; isDate(' + day + ',' + month + ',' + year + ');return false;"';}function isDate(dayVal,monthVal,yearVal) {var ct = nextDate(yearVal + getmmdd(monthVal+1,dayVal));if (ct == '') ct = nextDate('0000' + getmmdd(monthVal+1,dayVal));if (ct == '') ct = noMessage;document.getElementById('calDate').innerHTML = ct; return false;}function nextDate(yymmdd) {var x = dA.length;for (var i = 0; i < x; i++) {if (dA[i].substr(0,8) == yymmdd) return dA[i].substr(8);}return '';}function getmmdd(mm,dd) {return (mm > 9 ? '' + mm : '0' + mm) + (dd > 9 ? dd : '0' + dd);}
    	...
    	<div id="templatemo_content_area">
        	...
        	<div id="templaetmo_left">
                ...
                <div class="templatemo_menu">
                	<ul>
						<li><a href="#" class="current" onclick="facultyhome('home')">Home</a></li>
						<li><a href="#" class="current" onclick="facultyhome('event')">Add/Edit Events</a></li>
						<li><a href="#" class="current" onclick="facultyhome('delete')">Delete Events</a></li>
						<li><a href="#" onclick="facultyhome('password')">Change Password</a></li>	
...
Event  Already Exist  Try Again<%} %>
        	<div id="calendar" style="top: 120px;right: 30px;position: absolute;">
								<div id="calDate" style="border: 1pt solid #cccccc; width: 238px;color:#2F4F4F">
									2013-06-29:<br> No event messages to display for today
								</div>
								<div id="cal">
									<div id="cal">
										...
										<table border="0" cellspacing="0" cellpadding="2" width="210">
											<tbody>
												<tr>
													<td><a href="http://javascript.about.com/library/blevent1.htm#"
        	...
        	<div class="cleaner"></div>
                ...
                <div id="middle" style="top: 170px;position: absolute; left: 500px;" ></div>
...
int count=0;
String name=null,eno=null,eno1=null;
while(rs1.next())
{
	name=rs1.getString(1);
	eno=rs1.getString(2);
	eno1=rs1.getString(4);
	count++;
}
if(count==1)
{
	%>
	<marquee>Today Event:<%=name %> on <%=eno %> Time <%=eno1 %></marquee><% 
